...
def train(X_train, Y_train, X_test, Y_test):
	...
	for i, rate in enumerate(learning_rates):
		for j, reg in enumerate(regularization_strengths):
			...
			start = time.time()
			svm = LinearSVM()
			loss_history, acc_history = svm.train(X_train, Y_train, learning_rate=rate, reg=reg, num_iter=1500)
			Y_train_pred = svm.test(X_train)
			Y_test_pred = svm.test(X_test)
	...
	return best_svm, best_loss, best_train, best_val, best_lr, best_reg, best_y_pred, loss, results
...
if __name__ == '__main__':
	...
	best_svm, loss_history, best_train, best_val, best_lr, best_reg, best_y_pred, loss, result = train(X_train, Y_train, X_test, Y_test)	
	print(best_y_pred)
